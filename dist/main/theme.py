...
from os import stat
import cPickle 
from makelogobin import bootLogoHex
from os import walk
from os import path
...
def adbAvailable(config):
   stdin, stdout, stderr = popen3(config['adbCmd'] + ' version') # Check for adb
   if stderr.read() != '':
      if config['adbCmd'] == 'adb':
         ...
         if response == '':
            ...
            adbAvailable(config) # Check again with the updated config
      return False   
   else:
...
def getNinePatchImgs(path=""):
   stdout = popen("dir "+path)
   out = [x.split()[-1] for x in stdout.readlines() if len(x) >= 8 and x[-7:-1] == '.9.png']
   return out
...
def listApks():   
   stdout = popen('dir '+wk+'*.apk')
   apks = [x.split()[-1] for x in stdout.readlines() if len(x) >= 5 and x[-5:-1] == '.apk']
   return apks
...
menuOptions = ["Set current project", 
               ...
               "Create a .bin boot logo from a 24-bit BMP image (Droid2 and Droid X only)"]
...
menuFunctions = [setCurrentProject, setSysEnv, grabNinePatchImgs,
                 ...
                 decompile, compile, sign, compile9Patch, bootLogoHex]

while True:
   try: 
      stat(wk+str(config['currentProject']))
   except WindowsError:
